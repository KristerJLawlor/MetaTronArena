...
Shader "SineVFX/ForceFieldAndShieldEffects/ForceFieldDistortionHDRP"
{
	Properties
	{
		[HideInInspector] _EmissionColor("Emission Color", Color) = (1,1,1,1)
		[HideInInspector] _AlphaCutoff("Alpha Cutoff ", Range(0, 1)) = 0.5
		[ASEBegin]_MaskFresnelExp("Mask Fresnel Exp", Range( 0.2 , 8)) = 4
		...
		_ScreenDistortionPowerHDRP("Screen Distortion Power HDRP", Range( 0 , 2000)) = 1000
		...
		[ASEEnd][Toggle(_MASKAPPEARUSEWORLDPOSITION_ON)] _MaskAppearUseWorldPosition("Mask Appear Use World Position", Float) = 0
	...
	SubShader
	{
		...
		Pass
		{
			...
			float _ScreenDistortionPowerHDRP;
						...
						)
			{
				...
				float3 temp_output_57_0 = abs( normalWS );
				...
				transform130.xyz = GetAbsolutePositionWS((transform130).xyz);
				...
				float4 triplanar34 = TriplanarSampling34( _NoiseDistortion, ase_worldPos, normalWS, 1.0, _NoiseDistortionTiling, 1.0, 0 );
				...
				float3 switchResult27 = (((isFrontFace>0)?(float3(0,0,1)):(float3(0,0,-1))));
				...
				float clampResult206 = clamp( ( 1.0 - dotResult1 ) , 0.0 , 1.0 );
				...
				#endif
				float temp_output_10_0 = max( pow( clampResult206 , _MaskFresnelExp ) , staticSwitch167 );
				float clampResult80 = clamp( temp_output_10_0 , 0.0 , 1.0 );
				float clampResult207 = clamp( ( clampResult80 * _MaskSoftBordersMultiply ) , 0.0 , 1.0 );
				float clampResult125 = clamp( ( clampResult80 - pow( clampResult207 , _MaskSoftBordersExp ) ) , 0.0 , 1.0 );
				...
				#else
				float staticSwitch162 = packedInput.ase_texcoord7.xyz.y;
				#endif
				...
				#endif
				float2 uv_MaskAppearNoise = packedInput.ase_texcoord8.xy * _MaskAppearNoise_ST.xy + _MaskAppearNoise_ST.zw;
				float4 triplanar148 = TriplanarSampling148( _MaskAppearNoise, ase_worldPos, normalWS, 1.0, _MaskAppearNoiseTriplanarTiling, 1.0, 0 );
				...
				float temp_output_105_0 = ( ResultNoise77 * _ScreenDistortionPower * ResultOpacity93 * _ScreenDistortionPowerHDRP * clampResult161 );
				float2 temp_cast_3 = (temp_output_105_0).xx;
				...
				surfaceDescription.Distortion = temp_cast_3;
				surfaceDescription.DistortionBlur = 0.0;
				#endif
		...
		Pass
		{
			...
			void BuildSurfaceData(FragInputs fragInputs, inout DistortionSurfaceDescription surfaceDescription, float3 V, PositionInputs posInput, out SurfaceData surfaceData, out float3 bentNormalWS)
			{
				ZERO_INITIALIZE(SurfaceData, surfaceData);
			...
			PackedVaryingsMeshToPS VertexFunction(AttributesMesh inputMesh  )
			{
				...
				outputPackedVaryingsMeshToPS.ase_texcoord.xyz = ase_worldNormal;
			...
			float4 Frag(PackedVaryingsMeshToPS packedInput  ) : SV_Target
			{
				...
				float staticSwitch162 = ( FinalWorldPosition135.y / ase_objectScale.y );
				#else
				float staticSwitch162 = packedInput.ase_texcoord5.xyz.y;
				#endif
				float temp_output_146_0 = abs( ( (0.0 + (staticSwitch162 - 0.0) * (1.0 - 0.0) / (_MaskAppearLocalYRemap - 0.0)) + _MaskAppearLocalYAdd ) );
				#ifdef _MASKAPPEARINVERT_ON
				float staticSwitch153 = ( 1.0 - temp_output_146_0 );
				...
				float4 triplanar148 = TriplanarSampling148( _MaskAppearNoise, ase_worldPos, ase_worldNormal, 1.0, _MaskAppearNoiseTriplanarTiling, 1.0, 0 );
				#ifdef _MASKAPPEARNOISETRIPLANAR_ON
				float staticSwitch149 = triplanar148.x;
				#else
				float staticSwitch149 = tex2D( _MaskAppearNoise, uv_MaskAppearNoise ).r;
...
/*ASEBEGIN
Version=18934
1920;0;1920;1019;1983.605;63.69006;1;True;False
...
Node;AmplifyShaderEditor.RangedFloatNode;119;1249.683,348.8459;Float;False;Property;_NormalVertexOffset;Normal Vertex Offset;7;0;Create;True;0;0;0;False;0;False;0;0.02;0;0;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;155;3352.363,2410.409;Float;False;Constant;_Float6;Float 6;28;0;Create;True;0;0;0;False;0;False;0;0;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.StaticSwitch;149;2442.138,2681.209;Float;False;Property;_MaskAppearNoiseTriplanar;Mask Appear Noise Triplanar;30;0;Create;True;0;0;0;False;0;False;0;0;0;True;;Toggle;2;Key0;Key1;Create;False;True;All;9;1;FLOAT;0;False;0;FLOAT;0;False;2;FLOAT;0;False;3;FLOAT;0;False;4;FLOAT;0;False;5;FLOAT;0;False;6;FLOAT;0;False;7;FLOAT;0;False;8;FLOAT;0;False;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;151;2892.069,2230.508;Float;False;Property;_MaskAppearProgress;Mask Appear Progress;27;0;Create;True;0;0;0;False;0;False;0;-0.5644736;-2;2;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;140;2014.197,2283.544;Float;False;Property;_MaskAppearLocalYAdd;Mask Appear Local Y Add;25;0;Create;True;0;0;0;False;0;False;0;-0.5;0;0;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.StaticSwitch;134;-4127.17,614.1926;Float;False;Property;_LocalNoisePosition;Local Noise Position;8;0;Create;True;0;0;0;False;0;False;0;0;0;True;;Toggle;2;Key0;Key1;Create;False;True;All;9;1;FLOAT3;0,0,0;False;0;FLOAT3;0,0,0;False;2;FLOAT3;0,0,0;False;3;FLOAT3;0,0,0;False;4;FLOAT3;0,0,0;False;5;FLOAT3;0,0,0;False;6;FLOAT3;0,0,0;False;7;FLOAT3;0,0,0;False;8;FLOAT3;0,0,0;False;1;FLOAT3;0
...
Node;AmplifyShaderEditor.RangedFloatNode;138;1789.083,2185.887;Float;False;Property;_MaskAppearLocalYRemap;Mask Appear Local Y Remap;24;0;Create;True;0;0;0;False;0;False;0.5;0.5;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.StaticSwitch;162;1851.936,1812.683;Float;False;Property;_MaskAppearUseWorldPosition;Mask Appear Use World Position;32;0;Create;True;0;0;0;False;0;False;0;0;0;True;;Toggle;2;Key0;Key1;Create;False;True;All;9;1;FLOAT;0;False;0;FLOAT;0;False;2;FLOAT;0;False;3;FLOAT;0;False;4;FLOAT;0;False;5;FLOAT;0;False;6;FLOAT;0;False;7;FLOAT;0;False;8;FLOAT;0;False;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;145;1472.893,2865.109;Float;False;Property;_MaskAppearNoiseTriplanarTiling;Mask Appear Noise Triplanar Tiling;31;0;Create;True;0;0;0;False;0;False;0.2;0.1;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.TexturePropertyNode;143;1576.571,2654.796;Float;True;Property;_MaskAppearNoise;Mask Appear Noise;28;0;Create;True;0;0;0;False;0;False;None;3c4ea3a033cf4ed09936c7bd439486a4;False;white;Auto;Texture2D;-1;0;2;SAMPLER2D;0;SAMPLERSTATE;1
...
Node;AmplifyShaderEditor.RangedFloatNode;186;-126.7877,-265.6007;Float;False;Property;_ScreenDistortionPowerHDRP;Screen Distortion Power HDRP;23;0;Create;True;0;0;0;False;0;False;1000;1000;0;2000;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;40;-2737.115,2276.125;Float;False;Property;_Noise02ScrollSpeed;Noise 02 Scroll Speed;17;0;Create;True;0;0;0;False;0;False;0.25;0.1;0;0;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;32;-3620.341,1615.138;Float;False;Property;_NoiseDistortionTiling;Noise Distortion Tiling;21;0;Create;True;0;0;0;False;0;False;0.5;0.15;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.ClampOpNode;206;-2110.146,-274.1173;Inherit;False;3;0;FLOAT;0;False;1;FLOAT;0;False;2;FLOAT;1;False;1;FLOAT;0
Node;AmplifyShaderEditor.Vector3Node;30;-3139.04,114.9353;Float;False;Constant;_Vector1;Vector 1;3;0;Create;True;0;0;0;False;0;False;0,0,-1;0,0,0;0;4;FLOAT3;0;FLOAT;1;FLOAT;2;FLOAT;3
...
Node;AmplifyShaderEditor.RangedFloatNode;188;1294.561,63.32587;Float;False;Constant;_Float4;Float 4;35;0;Create;True;0;0;0;False;0;False;0;0;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;192;1296.669,-42.46557;Float;False;Constant;_Float5;Float 5;36;0;Create;True;0;0;0;False;0;False;0;0;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.AbsOpNode;57;-1080.388,1672.338;Inherit;False;1;0;FLOAT3;0,0,0;False;1;FLOAT3;0
Node;AmplifyShaderEditor.StaticSwitch;75;-395.7968,2137.46;Float;False;Property;_Noise02Enabled;Noise 02 Enabled;14;0;Create;True;0;0;0;False;0;False;0;0;0;True;;Toggle;2;Key0;Key1;Create;False;True;All;9;1;FLOAT;0;False;0;FLOAT;0;False;2;FLOAT;0;False;3;FLOAT;0;False;4;FLOAT;0;False;5;FLOAT;0;False;6;FLOAT;0;False;7;FLOAT;0;False;8;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.StaticSwitch;42;-2636.041,1614.004;Float;False;Property;_NoiseDistortionEnabled;Noise Distortion Enabled;18;0;Create;True;0;0;0;False;0;False;0;1;1;True;;Toggle;2;Key0;Key1;Create;False;True;All;9;1;FLOAT4;0,0,0,0;False;0;FLOAT4;0,0,0,0;False;2;FLOAT4;0,0,0,0;False;3;FLOAT4;0,0,0,0;False;4;FLOAT4;0,0,0,0;False;5;FLOAT4;0,0,0,0;False;6;FLOAT4;0,0,0,0;False;7;FLOAT4;0,0,0,0;False;8;FLOAT4;0,0,0,0;False;1;FLOAT4;0
...
Node;AmplifyShaderEditor.RangedFloatNode;33;-3182.934,1666.395;Float;False;Property;_NoiseDistortionPower;Noise Distortion Power;20;0;Create;True;0;0;0;False;0;False;0.5;1;0;2;0;1;FLOAT;0
Node;AmplifyShaderEditor.TexturePropertyNode;66;-1629.734,1406.841;Float;True;Property;_Noise01;Noise 01;11;0;Create;True;0;0;0;False;0;False;None;944be4a819c1444a960aa9df0803919d;True;white;Auto;Texture2D;-1;0;2;SAMPLER2D;0;SAMPLERSTATE;1
...
Node;AmplifyShaderEditor.ScreenColorNode;97;827.2896,-701.9072;Float;False;Global;_GrabScreen0;Grab Screen 0;21;0;Create;True;0;0;0;False;0;False;Object;-1;False;False;False;False;2;0;FLOAT2;0,0;False;1;FLOAT;0;False;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
...
Node;AmplifyShaderEditor.RangedFloatNode;16;-2809.876,508.0444;Float;False;Property;_MaskDepthFadeExp;Mask Depth Fade Exp;3;0;Create;True;0;0;0;False;0;False;4;2;0.2;10;0;1;FLOAT;0
Node;AmplifyShaderEditor.StaticSwitch;167;-2286.338,501.2999;Float;False;Property;_MaskDepthFadeEnabled;Mask Depth Fade Enabled;1;0;Create;True;0;0;0;False;0;False;0;1;1;True;;Toggle;2;Key0;Key1;Create;False;True;All;9;1;FLOAT;0;False;0;FLOAT;0;False;2;FLOAT;0;False;3;FLOAT;0;False;4;FLOAT;0;False;5;FLOAT;0;False;6;FLOAT;0;False;7;FLOAT;0;False;8;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.StaticSwitch;116;1288.873,-387.8796;Float;False;Property;_VisualizeMaskDebug;Visualize Mask Debug;33;0;Create;True;0;0;0;False;0;False;0;0;0;True;;Toggle;2;Key0;Key1;Create;False;True;All;9;1;COLOR;0,0,0,0;False;0;COLOR;0,0,0,0;False;2;COLOR;0,0,0,0;False;3;COLOR;0,0,0,0;False;4;COLOR;0,0,0,0;False;5;COLOR;0,0,0,0;False;6;COLOR;0,0,0,0;False;7;COLOR;0,0,0,0;False;8;COLOR;0,0,0,0;False;1;COLOR;0
Node;AmplifyShaderEditor.GetLocalVarNode;79;-42.28455,-524.7101;Inherit;False;77;ResultNoise;1;0;OBJECT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.SamplerNode;157;3732.961,2241.487;Inherit;True;Property;_MaskAppearRamp;Mask Appear Ramp;29;0;Create;True;0;0;0;False;0;False;-1;None;2128559934d148e5bfac51ec6019d89c;True;0;False;white;Auto;False;Object;-1;Auto;Texture2D;8;0;SAMPLER2D;;False;1;FLOAT2;0,0;False;2;FLOAT;0;False;3;FLOAT2;0,0;False;4;FLOAT2;0,0;False;5;FLOAT;1;False;6;FLOAT;0;False;7;SAMPLERSTATE;;False;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.RangedFloatNode;123;-1264.202,571.6525;Float;False;Property;_MaskSoftBordersExp;Mask Soft Borders Exp;6;0;Create;True;0;0;0;False;0;False;20;20;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.TexturePropertyNode;31;-3604.521,1422.811;Float;True;Property;_NoiseDistortion;Noise Distortion;19;0;Create;True;0;0;0;False;0;False;None;66819fafe6a64db797e1a5f6086c487f;False;white;Auto;Texture2D;-1;0;2;SAMPLER2D;0;SAMPLERSTATE;1
Node;AmplifyShaderEditor.TexturePropertyNode;55;-1591.566,2339.09;Float;True;Property;_Noise02;Noise 02;15;0;Create;True;0;0;0;False;0;False;None;944be4a819c1444a960aa9df0803919d;True;white;Auto;Texture2D;-1;0;2;SAMPLER2D;0;SAMPLERSTATE;1
Node;AmplifyShaderEditor.RangedFloatNode;127;-1427.053,438.101;Float;False;Property;_MaskSoftBordersMultiply;Mask Soft Borders Multiply;5;0;Create;True;0;0;0;False;0;False;1.01;1.01;0;0;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.TexturePropertyNode;174;371.4029,-972.4889;Float;True;Global;_CameraOpaqueTexture;_CameraOpaqueTexture;34;0;Create;True;0;0;0;False;0;False;None;None;False;white;Auto;Texture2D;-1;0;2;SAMPLER2D;0;SAMPLERSTATE;1
Node;AmplifyShaderEditor.PowerNode;15;-2489.671,436.0445;Inherit;False;False;2;0;FLOAT;0;False;1;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.StaticSwitch;153;2872.5,2110.995;Float;False;Property;_MaskAppearInvert;Mask Appear Invert;26;0;Create;True;0;0;0;False;0;False;0;0;0;True;;Toggle;2;Key0;Key1;Create;False;True;All;9;1;FLOAT;0;False;0;FLOAT;0;False;2;FLOAT;0;False;3;FLOAT;0;False;4;FLOAT;0;False;5;FLOAT;0;False;6;FLOAT;0;False;7;FLOAT;0;False;8;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.GetLocalVarNode;160;666.5569,-165.0152;Inherit;False;159;MaskAppearValue;1;0;OBJECT;0;False;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;36;-2853.14,1677.703;Float;False;Constant;_Float3;Float 3;23;0;Create;True;0;0;0;False;0;False;0;0;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;47;-2771.651,1348.873;Float;False;Property;_Noise01ScrollSpeed;Noise 01 Scroll Speed;13;0;Create;True;0;0;0;False;0;False;0.25;0.25;0;0;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;168;-2485.079,615.9754;Float;False;Constant;_Float0;Float 0;33;0;Create;True;0;0;0;False;0;False;0;0;0;0;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;96;-1776.266,318.2471;Float;False;Property;_NoiseMaskAdd;Noise Mask Add;10;0;Create;True;0;0;0;False;0;False;0.25;1;0;2;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.SamplerNode;63;-1252.089,1896.542;Inherit;True;Property;_TextureSample2;Texture Sample 2;15;0;Create;True;0;0;0;False;0;False;-1;None;None;True;0;False;white;Auto;True;Object;-1;Auto;Texture2D;8;0;SAMPLER2D;;False;1;FLOAT2;0,0;False;2;FLOAT;0;False;3;FLOAT2;0,0;False;4;FLOAT2;0,0;False;5;FLOAT;1;False;6;FLOAT;0;False;7;SAMPLERSTATE;;False;5;FLOAT3;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.Vector4Node;129;-5085.167,835.3986;Float;False;Constant;_Vector2;Vector 2;22;0;Create;True;0;0;0;False;0;False;0,0,0,1;0,0,0,0;0;5;FLOAT4;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
...
Node;AmplifyShaderEditor.ClampOpNode;161;902.004,-162.9189;Inherit;False;3;0;FLOAT;0;False;1;FLOAT;0;False;2;FLOAT;1;False;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;103;-109.0102,-440.3779;Float;False;Property;_ScreenDistortionPower;Screen Distortion Power;22;0;Create;True;0;0;0;False;0;False;0.025;0.015;0;0.2;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.Vector3Node;29;-3134.244,-49.72601;Float;False;Constant;_Vector0;Vector 0;3;0;Create;True;0;0;0;False;0;False;0,0,1;0,0,0;0;4;FLOAT3;0;FLOAT;1;FLOAT;2;FLOAT;3
...
Node;AmplifyShaderEditor.SamplerNode;72;-1290.257,964.294;Inherit;True;Property;_TextureSample5;Texture Sample 5;15;0;Create;True;0;0;0;False;0;False;-1;None;None;True;0;False;white;Auto;True;Object;-1;Auto;Texture2D;8;0;SAMPLER2D;;False;1;FLOAT2;0,0;False;2;FLOAT;0;False;3;FLOAT2;0,0;False;4;FLOAT2;0,0;False;5;FLOAT;1;False;6;FLOAT;0;False;7;SAMPLERSTATE;;False;5;FLOAT3;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
...
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;105;377.3232,-548.277;Inherit;False;5;5;0;FLOAT;0;False;1;FLOAT;0;False;2;FLOAT;0;False;3;FLOAT;0;False;4;FLOAT;0;False;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;74;-594.3628,1697.967;Float;False;Property;_NoiseMaskPower;Noise Mask Power;9;0;Create;True;0;0;0;False;0;False;1;2;0;10;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;13;-3316.347,386.8741;Float;False;Property;_MaskDepthFadeDistance;Mask Depth Fade Distance;2;0;Create;True;0;0;0;False;0;False;6;12;0;0;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.StaticSwitch;128;-415.7776,171.1426;Float;False;Property;_MaskSoftBorders;Mask Soft Borders;4;0;Create;True;0;0;0;False;0;False;0;0;0;True;;Toggle;2;Key0;Key1;Create;False;True;All;9;1;FLOAT;0;False;0;FLOAT;0;False;2;FLOAT;0;False;3;FLOAT;0;False;4;FLOAT;0;False;5;FLOAT;0;False;6;FLOAT;0;False;7;FLOAT;0;False;8;FLOAT;0;False;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;7;-2390.876,-78.52291;Float;False;Property;_MaskFresnelExp;Mask Fresnel Exp;0;0;Create;True;0;0;0;False;0;False;4;0.2;0.2;8;0;1;FLOAT;0
Node;AmplifyShaderEditor.SamplerNode;61;-1252.09,2091.542;Inherit;True;Property;_TextureSample0;Texture Sample 0;15;0;Create;True;0;0;0;False;0;False;-1;None;None;True;0;False;white;Auto;True;Object;-1;Auto;Texture2D;8;0;SAMPLER2D;;False;1;FLOAT2;0,0;False;2;FLOAT;0;False;3;FLOAT2;0,0;False;4;FLOAT2;0,0;False;5;FLOAT;1;False;6;FLOAT;0;False;7;SAMPLERSTATE;;False;5;FLOAT3;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
...
Node;AmplifyShaderEditor.RangedFloatNode;68;-608.4453,2228.068;Float;False;Constant;_Float8;Float 8;24;0;Create;True;0;0;0;False;0;False;1;0;0;0;0;1;FLOAT;0
...
WireConnection;105;3;186;0
WireConnection;105;4;161;0
...
WireConnection;122;0;207;0
WireConnection;122;1;123;0
WireConnection;95;0;17;0
WireConnection;95;1;96;0
WireConnection;71;0;66;0
WireConnection;71;1;65;0
WireConnection;189;0;161;0
WireConnection;189;1;188;0
WireConnection;175;0;174;0
WireConnection;175;1;100;0
WireConnection;207;0;126;0
WireConnection;194;9;188;0
WireConnection;194;28;105;0
WireConnection;194;29;192;0
WireConnection;194;11;118;0
ASEEND*/
//CHKSM=05C16B8243F299D8D793B011007174A395297D0C