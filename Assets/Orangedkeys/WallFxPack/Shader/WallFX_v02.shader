	...
	SubShader
	{
		...
		Pass
		{
			...
			VertexOutput VertexFunction( VertexInput inputMesh  )
			{
				...
				float2 uv2_DistortionMask = inputMesh.ase_texcoord1.xy * _DistortionMask_ST.xy + _DistortionMask_ST.zw;
				float2 texCoord1 = inputMesh.ase_texcoord1.xy * float2( 1,1 ) + ( (( _AnimatedOpening )?( ( float2( 1,1 ) * sin( mulTime69 ) ) ):( temp_cast_0 )) + ( tex2Dlod( _DistortionMask, float4( uv2_DistortionMask, 0, 0.0) ).r * _MaskDistortion ) );
				float saferPower27 = max( texCoord1.y , 0.0001 );
				float clampResult24 = clamp( pow( saferPower27 , 5.0 ) , 0.0 , 1.0 );
				float lerpResult19 = lerp( 0.0 , inputMesh.ase_color.g , ( clampResult24 * ( -2.0 * _StretchingAnime ) ));
				...
				#endif
				float3 vertexValue = ( appendResult21 * inputMesh.ase_color.b ).xyz;
				#ifdef ASE_ABSOLUTE_VERTEX_POS
			...
			float4 Frag( VertexOutput packedInput ) : SV_Target
			{
				...
				float temp_output_57_0 = ( packedInput.ase_color.r * _OutlinesMult );
				float4 screenPos = packedInput.ase_texcoord1;
				float4 ase_grabScreenPos = ASE_ComputeGrabScreenPos( screenPos );
				float4 ase_grabScreenPosNorm = ase_grabScreenPos / ase_grabScreenPos.w;
				float4 fetchOpaqueVal43 = ASEHDSampleSceneColor(ase_grabScreenPosNorm.xy, 0, GetInverseCurrentExposureMultiplier());
				...
				float mulTime69 = _TimeParameters.x * _SpeedAnimatedMode;
				float2 uv2_DistortionMask = packedInput.ase_texcoord2.xy * _DistortionMask_ST.xy + _DistortionMask_ST.zw;
				float2 texCoord1 = packedInput.ase_texcoord2.xy * float2( 1,1 ) + ( (( _AnimatedOpening )?( ( float2( 1,1 ) * sin( mulTime69 ) ) ):( temp_cast_3 )) + ( tex2D( _DistortionMask, uv2_DistortionMask ).r * _MaskDistortion ) );
				float saferPower27 = max( texCoord1.y , 0.0001 );
				float clampResult24 = clamp( pow( saferPower27 , 5.0 ) , 0.0 , 1.0 );
				...
				float temp_output_62_0 = ( 1.0 - packedInput.ase_color.b );
				float3 lerpResult63 = lerp( ( ( packedInput.ase_color.r * _WallOutlineColorMult * localMyCustomExpression80 ) + lerpResult40 ) , localMyCustomExpression80 , temp_output_62_0);
				
				surfaceDescription.Color = lerpResult63;
				surfaceDescription.Emission = lerpResult63;
				surfaceDescription.Alpha = ( ( pow( temp_output_36_0 , 0.2 ) * packedInput.ase_color.b ) + ( temp_output_62_0 * (( _DisplayFrame )?( _FrameEmissive ):( 0.0 )) ) );
		...
		Pass
		{
					...
					)
			{
				...
				float2 temp_cast_0 = (( ( _WallOpening - 0.5 ) * 2.0 )).xx;
				float mulTime69 = _TimeParameters.x * _SpeedAnimatedMode;
				float2 uv2_DistortionMask = packedInput.ase_texcoord.xy * _DistortionMask_ST.xy + _DistortionMask_ST.zw;
				float2 texCoord1 = packedInput.ase_texcoord.xy * float2( 1,1 ) + ( (( _AnimatedOpening )?( ( float2( 1,1 ) * sin( mulTime69 ) ) ):( temp_cast_0 )) + ( tex2D( _DistortionMask, uv2_DistortionMask ).r * _MaskDistortion ) );
				float saferPower27 = max( texCoord1.y , 0.0001 );
				float clampResult24 = clamp( pow( saferPower27 , 5.0 ) , 0.0 , 1.0 );
				float temp_output_36_0 = ( 1.0 - clampResult24 );
				float temp_output_62_0 = ( 1.0 - packedInput.ase_color.b );
		...
		Pass
		{
			...
			VertexOutput VertexFunction( VertexInput inputMesh  )
			{
				...
				float2 temp_cast_0 = (( ( _WallOpening - 0.5 ) * 2.0 )).xx;
				float mulTime69 = _TimeParameters.x * _SpeedAnimatedMode;
				float2 uv2_DistortionMask = inputMesh.uv1.xy * _DistortionMask_ST.xy + _DistortionMask_ST.zw;
				float2 texCoord1 = inputMesh.uv1.xy * float2( 1,1 ) + ( (( _AnimatedOpening )?( ( float2( 1,1 ) * sin( mulTime69 ) ) ):( temp_cast_0 )) + ( tex2Dlod( _DistortionMask, float4( uv2_DistortionMask, 0, 0.0) ).r * _MaskDistortion ) );
				float saferPower27 = max( texCoord1.y , 0.0001 );
				float clampResult24 = clamp( pow( saferPower27 , 5.0 ) , 0.0 , 1.0 );
				float lerpResult19 = lerp( 0.0 , inputMesh.ase_color.g , ( clampResult24 * ( -2.0 * _StretchingAnime ) ));
				float4 appendResult21 = (float4(0.0 , lerpResult19 , 0.0 , 0.0));
			...
			float4 Frag( VertexOutput packedInput  ) : SV_Target
			{
				...
				float2 temp_cast_3 = (( ( _WallOpening - 0.5 ) * 2.0 )).xx;
				float mulTime69 = _TimeParameters.x * _SpeedAnimatedMode;
				float2 uv2_DistortionMask = packedInput.ase_texcoord1.xy * _DistortionMask_ST.xy + _DistortionMask_ST.zw;
				float2 texCoord1 = packedInput.ase_texcoord1.xy * float2( 1,1 ) + ( (( _AnimatedOpening )?( ( float2( 1,1 ) * sin( mulTime69 ) ) ):( temp_cast_3 )) + ( tex2D( _DistortionMask, uv2_DistortionMask ).r * _MaskDistortion ) );
				float saferPower27 = max( texCoord1.y , 0.0001 );
				float clampResult24 = clamp( pow( saferPower27 , 5.0 ) , 0.0 , 1.0 );
				float temp_output_36_0 = ( 1.0 - clampResult24 );
				float3 lerpResult40 = lerp( ( temp_output_57_0 + ( localMyCustomExpression80 * _EmissiveMult ) ) , ( pow( desaturateVar54 , 1.0 ) + ( localMyCustomExpression80 * _WallColorMult ) ) , pow( temp_output_36_0 , 10.0 ));
		...
		Pass
		{
					...
					)
			{
				...
				float2 temp_cast_0 = (( ( _WallOpening - 0.5 ) * 2.0 )).xx;
				float mulTime69 = _TimeParameters.x * _SpeedAnimatedMode;
				float2 uv2_DistortionMask = packedInput.ase_texcoord3.xy * _DistortionMask_ST.xy + _DistortionMask_ST.zw;
				float2 texCoord1 = packedInput.ase_texcoord3.xy * float2( 1,1 ) + ( (( _AnimatedOpening )?( ( float2( 1,1 ) * sin( mulTime69 ) ) ):( temp_cast_0 )) + ( tex2D( _DistortionMask, uv2_DistortionMask ).r * _MaskDistortion ) );
				float saferPower27 = max( texCoord1.y , 0.0001 );
				float clampResult24 = clamp( pow( saferPower27 , 5.0 ) , 0.0 , 1.0 );
				float temp_output_36_0 = ( 1.0 - clampResult24 );
				float temp_output_62_0 = ( 1.0 - packedInput.ase_color.b );
...
/*ASEBEGIN
Version=18400
-1919;0;1920;1011;1161.038;405.3894;1.356312;True;False
...
WireConnection;106;0;63;0
WireConnection;106;1;63;0
WireConnection;106;2;61;0
WireConnection;106;6;60;0
ASEEND*/
//CHKSM=0365C1747A3FF6F05B9C2DE794DF426615B3BF82